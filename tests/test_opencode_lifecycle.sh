...
if ensure_worktrees_ignored "$repo"; then
  echo "OpenCode worktree guard accepted an unignored directory" >&2
  exit 1
fi
...
[[ "$mounts" == "$repo
$ROOT:ro" ]] || {
  echo "unexpected OpenCode workspace mounts: $mounts" >&2
  exit 1
}
...
if [[ "$1" == "template" && "$2" == "ls" ]]; then
  printf '%s\n' 'docker.io/library/opencode-sbx local 753b231c8eaa opencode-docker 2 days ago'
  exit 0
fi
...
(
  cd "$repo"
  PATH="$tmp/bin:$PATH" bash -c "source '$ROOT/harnesses/opencode/bin/opencode-sbx'; main --model test-model"
)
...
(
  cd "$repo"
  PATH="$tmp/bin:$PATH" bash -c "source '$ROOT/harnesses/opencode/bin/opencode-sbx'; main --session resumed"
)
...
[[ "$(grep -Fc "sbx exec $name bash $ROOT/harnesses/opencode/scripts/bootstrap.sh" "$MOCK_LOG")" == 2 ]] || {
  cat "$MOCK_LOG" >&2
  exit 1
}
